...
function markup({ bodyClass = 'authenticated theme-initializing', preferredTheme = 'light', preferredThemeNull = false, userId = 'u1', currentCulture = 'fr', preferredCulture = 'fr', sidebarClass = 'sidebar' } = {}) {
    return `

<body class="${bodyClass}">
<input type="hidden" name="__RequestVerificationToken" value="token">
<aside class="${sidebarClass}" data-preferred-culture="${preferredCulture}" data-preferred-theme="${preferredTheme}" ${preferredThemeNull ? 'data-preferred-theme-null="true"' : ''} data-current-culture="${currentCulture}" data-user-id="${userId}" data-update-preferences-url="/prefs">
 <button class="toggle"></button><button class="sidebar-open-btn"></button><button class="sidebar-close-btn"></button>
 <ul>
  <li class="menu" data-menu-id="one"><span class="menu-target">One</span></li>
  <li class="menu" data-menu-id="two"><span class="menu-target-2">Two</span></li>
 </ul>
...
describe('sidebar.js', () => {
    ...
    it('persists the sidebar state when toggling it', async () => {
        ...
        expect($('.sidebar').hasClass('close')).toBe(true);
        expect(localStorage.getItem('sidebar-state')).toBe('closed');

        $('.toggle').trigger('click');
        expect($('.sidebar').hasClass('close')).toBe(false);
        expect(localStorage.getItem('sidebar-state')).toBe('open');
    });

    it('restores an open sidebar from local storage', async () => {
    ...
    it('does not restore a closed sidebar from local storage', async () => {
        ...
        expect($('.sidebar').hasClass('close')).toBe(false);
        expect($('.sidebar-open-btn').hasClass('btn-visible')).toBe(false);
        expect($('.sidebar-close-btn').hasClass('btn-visible')).toBe(false);
    });

    it('persists the currently open menu', async () => {
        await loadScript('sidebar.js');
        ...
        expect($('.menu').eq(0).hasClass('show')).toBe(false);
        expect($('.menu').eq(1).hasClass('show')).toBe(true);
        expect(localStorage.getItem('sidebar-menu')).toBe('two');
    });

    it('removes the remembered menu when the current menu is closed', async () => {
        await loadScript('sidebar.js');
    ...
    it('restores the remembered menu when the sidebar is open', async () => {
        localStorage.setItem('sidebar-state', 'open');
        localStorage.setItem('sidebar-menu', 'two');
    ...
    it('does not restore the remembered menu when the sidebar is closed', async () => {
        localStorage.setItem('sidebar-state', 'closed');
        localStorage.setItem('sidebar-menu', 'two');
        document.body.outerHTML = markup({ sidebarClass: 'sidebar close' }).trim();

        await loadScript('sidebar.js');

        expect($('.sidebar').hasClass('close')).toBe(true);
        expect($('.menu').eq(0).hasClass('show')).toBe(false);
        expect($('.menu').eq(1).hasClass('show')).toBe(false);
    });
