...
public sealed record AccountDto(Guid Id, string Name, bool IsClosed, string Iban, BankDto Bank);
public sealed record AccessDto(UserDto User, bool IsOwner, IReadOnlyList<Permission> Permissions);
public sealed record BudgetCategoryDto(Guid Id, string Name, string? Description, int Order, decimal Expenses, decimal Incomes, decimal Balance);
public sealed record TransactionDto(Guid Id, string Name, BudgetCategoryDto Category, TransactionType Type, decimal Amount, decimal SignedAmount, PaymentMethod Method, AccountDto Account, AccountDto? TransferAccount);
